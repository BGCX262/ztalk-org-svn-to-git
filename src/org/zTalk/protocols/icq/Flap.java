 ...
 */
package org.zTalk.protocols.icq;
...
public class Flap extends DataContainer {
	...
	/**
	 * This create an empty Flap. Only the command start (first byte) will be
	 * set.
	 */
	public Flap() {
	...
	/**
	 * Construct a Flap giving it a channel ID and a sequence number. If you use
	 * this constructor the Flap could not contains a Snac Section.
	 * 
	 * @param channelId
	 *            The channel the Flap belongs.
	 */
	...
	public byte[] getHeaderByteArray() {
		if (headerModified) {
			...
			position += commandStart.getByteArray().length;
			System.arraycopy(channelId.getByteArray(), 0, headerByteArray,
			...
			position += sequenceNumber.getByteArray().length;
			System.arraycopy(dataFieldLength.getByteArray(), 0,
					headerByteArray, position,
					dataFieldLength.getByteArray().length);
