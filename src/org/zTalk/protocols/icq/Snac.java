...
public class Snac extends DataContainer {
	...
	public Snac(byte array[], int start) {
		...
		System.arraycopy(array, start + 10, dataFieldArray, 0, array.length
				- (start + 10));
	...
	public byte[] getHeaderByteArray() {
		if (headerModified) {
			...
			System.arraycopy(familyId.getByteArray(), 0, headerByteArray,
					position, familyId.getByteArray().length);
			position += familyId.getByteArray().length;
			System.arraycopy(subTypeId.getByteArray(), 0, headerByteArray,
					position, familyId.getByteArray().length);
			position += familyId.getByteArray().length;
			System.arraycopy(flag0.getByteArray(), 0, headerByteArray,
					position, flag0.getByteArray().length);
			position += flag0.getByteArray().length;
			System.arraycopy(flag1.getByteArray(), 0, headerByteArray,
					position, flag1.getByteArray().length);
			position += flag1.getByteArray().length;
			System.arraycopy(requestId.getByteArray(), 0, headerByteArray,
					position, requestId.getByteArray().length);
