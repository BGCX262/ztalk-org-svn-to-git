...
public interface IContactList {
	...
	/**
	 * Добавляем новый контакт с UIN'ом <code>userId</code> в группу с названием
	 * <code>groupName</code>
	 * 
	 * @param userId
	 *            идентификатор добавляемого контакта
	 * @param groupName
	 *            название группы, в которую добавляется контакт
	...
	/**
	 * Добавляем новый контакт с UIN'ом <code>userId</code> в группу
	 * <code>group</code>
	 * 
	 * @param userId
	 *            идентификатор добавляемого контакта
	 * @param group
	 *            группа, в которую добавляется контакт
	 * @throws ContactListOperationException
	 */
	 ...
	 * 
	 * @param userId
	 *            уин пользователя, из листа которого мы себя удаляем
	 */
	public void removeYourself(String userId);

	/**
	 * Отправляем контакту с UIN <code>userId</code> запрос авторизации
	 * <code>request</code>
	 * 
	 * @param userId
	 *            UIN контакт, которому отправляем запрос авторизации
	 * @param request
	...
	/**
	 * Отправляем ответ <code>auth</code> с текстом <code>reply</code> на запрос
	 * авторизации от пользователя с UIN <code>userId</code>
	 * 
	 * @param userId
	 *            UIN пользователя, который запросил авторизацию
	 * @param reply
	 *            текст ответа
	 * @param auth
	 *            результат авторизации, если <code>true</code>, то пользователь
	 *            считается авторизованым
	 * @throws ContactListOperationException
	 */
	...
	/**
	 * Отправляем пользователю с UIN <code>userId</code> сообщение
	 * <i>"You were added"</i>
	 * 
	 * @param userId
	 *            получатель сообщения о добавлении
	 */
