...
import org.zTalk.protocols.icq.integration.events.XStatusRequestEvent;
import org.zTalk.protocols.icq.integration.events.XStatusResponseEvent;
...
import org.zTalk.protocols.icq.integration.listeners.XStatusListener;
 ...
 */
public class XStatusEventsTest implements MessagingListener, XStatusListener,
		OurStatusListener {
	...
	public void onIncomingMessage(IncomingMessageEvent e) {
		log.info("Incoming message from " + e.getSenderID() + ": "
				+ e.getMessage());
	...
	public void onXStatusRequest(XStatusRequestEvent e) {
		try {
			OscarInterface.sendXStatus(connection, new XStatusModeEnum(
					XStatusModeEnum.LOVE), XSTATUS_BASE_STRING,
					XSTATUS_EXT_STRING, e.getTime(), e.getMsgID(), e
							.getSenderID(), e.getSenderTcpVersion());
	...
	public void onXStatusResponse(XStatusResponseEvent e) {
		try {
			OscarInterface.sendExtendedMessage(
					connection,
					e.getSenderID(),
					"Title = " + e.getTitle() + " Description = "
							+ e.getDescription() + " XStatus = "
							+ e.getXStatus().toString());
		} catch (ConvertStringException ex) {
			log.error(ex.getMessage(), ex);
		}
	}
	...
	public void onLogin() {
		...
		OscarInterface.changeXStatus(connection, new XStatusModeEnum(
				XStatusModeEnum.LOVE));
	}
