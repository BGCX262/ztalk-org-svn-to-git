...
import org.zTalk.protocols.icq.integration.events.MetaBasicUserInfoEvent;
import org.zTalk.protocols.icq.integration.listeners.MetaInfoListener;
import org.zTalk.protocols.icq.setting.enumerations.CountryEnum;
import org.zTalk.protocols.icq.setting.enumerations.TimeZoneEnum;
import org.zTalk.protocols.icq.tool.StringTools;

/**
 * <p>
 * Created by 25.03.2008
 * 
...
public class BasicUserInfoParser extends BaseMetaInfoParser {
	...
	protected void sendMessage(EventListener listener, EventObject e) {
		...
		((MetaInfoListener) listener)
				.onBasicUserInfo((MetaBasicUserInfoEvent) e);
	}
	...
	public void parse(byte[] data, int position) throws ConvertStringException {
		...
		// Nickname
		nickName = StringTools.byteArrayToString(data, position,
				rStrLen.getValue() - 1);
		...
		// First Name
		firstName = StringTools.byteArrayToString(data, position,
				rStrLen.getValue() - 1);
		position += rStrLen.getValue();
		...
		// Last Name
		lastName = StringTools.byteArrayToString(data, position,
				rStrLen.getValue() - 1);
		position += rStrLen.getValue();
		...
		// Email
		email = (new RawData(data, position, rStrLen.getValue() - 1))
				.getStringValue();
		position += rStrLen.getValue();
		...
		// Home city
		homeCity = StringTools.byteArrayToString(data, position,
				rStrLen.getValue() - 1);
		position += rStrLen.getValue();
		...
		// Home state
		homeState = StringTools.byteArrayToString(data, position,
				rStrLen.getValue() - 1);
		position += rStrLen.getValue();
		...
		// Home phone
		homePhone = (new RawData(data, position, rStrLen.getValue() - 1))
				.getStringValue();
		position += rStrLen.getValue();
		...
		// Home fax
		homeFax = (new RawData(data, position, rStrLen.getValue() - 1))
				.getStringValue();
		position += rStrLen.getValue();
		...
		// Home address
		homeAddress = StringTools.byteArrayToString(data, position,
				rStrLen.getValue() - 1);
		position += rStrLen.getValue();
		...
		// Cell phone
		cellPhone = (new RawData(data, position, rStrLen.getValue() - 1))
				.getStringValue();
		position += rStrLen.getValue();
		...
		// Home zip
		zipCode = (new RawData(data, position, rStrLen.getValue() - 1))
				.getStringValue();
		position += rStrLen.getValue();

		// Country code
		RawData homeCountryRD = (new RawData(data, position,
				RawData.WORD_LENGHT));
		...
		// GMT offset
		timezone = (byte) (new RawData(data, position, RawData.BYTE_LENGHT))
				.getValue();
		...
		RawData publishemaildata = new RawData(data, position,
				RawData.BYTE_LENGHT);
