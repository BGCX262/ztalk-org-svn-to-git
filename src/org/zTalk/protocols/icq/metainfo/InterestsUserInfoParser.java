...
import org.zTalk.protocols.icq.setting.enumerations.InterestsEnum;
...
public class InterestsUserInfoParser extends BaseMetaInfoParser {
	...
	protected void sendMessage(EventListener listener, EventObject e) {
		...
		((MetaInfoListener) listener)
				.onInterestsUserInfo((MetaInterestsUserInfoEvent) e);
	}
	...
	public void parse(byte[] data, int position) throws ConvertStringException {
		...
		for (int i = 0; i < len; i++) {
			...
			// Interest
			String interest = StringTools.byteArrayToString(data, position,
					rStrLen.getValue() - 1);
