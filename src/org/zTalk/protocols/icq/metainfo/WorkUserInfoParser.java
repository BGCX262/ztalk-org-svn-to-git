...
import org.zTalk.protocols.icq.setting.enumerations.OccupationEnum;
...
public class WorkUserInfoParser extends BaseMetaInfoParser {
	...
	public void parse(byte[] data, int position) throws ConvertStringException {
		...
		// Work City
		workCity = StringTools.byteArrayToString(data, position,
				rStrLen.getValue() - 1);
		...
		// Work State
		workState = StringTools.byteArrayToString(data, position,
				rStrLen.getValue() - 1);
		position += rStrLen.getValue();
		...
		// Work Phone
		workPhone = StringTools.byteArrayToString(data, position,
				rStrLen.getValue() - 1);
		position += rStrLen.getValue();
		...
		// Work Fax
		workFax = StringTools.byteArrayToString(data, position,
				rStrLen.getValue() - 1);
		position += rStrLen.getValue();
		...
		// Work Address
		workAddress = StringTools.byteArrayToString(data, position,
				rStrLen.getValue() - 1);
		position += rStrLen.getValue();
		...
		// Work Zip code
		workZip = (new RawData(data, position, rStrLen.getValue() - 1))
				.getStringValue();
		position += rStrLen.getValue();
		...
		// Work Company
		workCompany = StringTools.byteArrayToString(data, position,
				rStrLen.getValue() - 1);
		position += rStrLen.getValue();
		...
		// Work Department
		workDepartment = StringTools.byteArrayToString(data, position,
				rStrLen.getValue() - 1);
		position += rStrLen.getValue();
		...
		// Work Position
		workPosition = StringTools.byteArrayToString(data, position,
				rStrLen.getValue() - 1);
		position += rStrLen.getValue();
		...
		// Work WebPage
		workWebPage = StringTools.byteArrayToString(data, position,
				rStrLen.getValue() - 1);
		position += rStrLen.getValue();
