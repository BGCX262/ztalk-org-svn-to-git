...
import org.zTalk.protocols.icq.core.exceptions.LoginException;
import org.zTalk.protocols.icq.setting.enumerations.LoginErrorTypeEnum;
...
public class AuthorizationReply extends ReceivedPacket {
	...
	/**
	 * Constructor of the representation of the Authorization Response packet
	 * received from mirabilis.
	 * 
	 * @param packet
	 *            The byte array received with the socket.
	 */
	public AuthorizationReply(byte[] packet) {
		super(packet, false);

		Tlv tempTlv = null;
		for (int position = 0; position < getDataFieldByteArray().length; position += tempTlv
				.getByteArray().length) {
	...
	public void connectToBos(OscarConnection connection) throws IOException {
		...
		OscarClient bosClient = new OscarClient(getBosServerAddress(),
				getBosServerPort(), connection.getPacketAnalyser());
		/* Set the new server */
	...
	public int getBosServerPort() {
		int dbpt = bosServerAddress.getStringValue().indexOf(':');
		return Integer.parseInt(bosServerAddress.getStringValue().substring(
				dbpt + 1, bosServerAddress.getStringValue().length()));
	}
	...
	/**
	 * This function returns the Url where the user can find help in case of
	 * troubble.
	 * 
	...
	private LoginErrorTypeEnum getErrorType() {
		if (errorCode != null) {
			...
			case 0x07:
			case 0x08:
				return new LoginErrorTypeEnum(
						LoginErrorTypeEnum.NOT_EXISTS_ERROR);

			case 0x16:
			case 0x17:
				return new LoginErrorTypeEnum(
						LoginErrorTypeEnum.MAXIMUM_USERS_IP_ERROR);

			case 0x18:
			case 0x1D:
				return new LoginErrorTypeEnum(
						LoginErrorTypeEnum.LIMIT_EXCEEDED_ERROR);

			case 0x1B:
			case 0x1C:
				return new LoginErrorTypeEnum(
						LoginErrorTypeEnum.OLDER_ICQ_VERSION_ERROR);

			case 0x1E:
				return new LoginErrorTypeEnum(
						LoginErrorTypeEnum.CANT_REGISTER_ERROR);
