...
import org.zTalk.protocols.icq.Tlv;
...
import org.zTalk.protocols.icq.packet.sent.icbm.MessageType2Ack;
...
public class IncomingMessage__4_7 extends ReceivedPacket {
	...
	public void execute(OscarConnection connection) throws Exception {
		...
		if (messageChannel.getValue() == 2 && !isRequestXStatus) {
			connection
					.sendFlap(new MessageType2Ack(time, msgId, senderID,
							senderTcpVer, msgSeqNum, msgType, msgFlags,
							connection.getTweaker().getRequestMessage(
									msgType.getValue())));
		}
	}
	...
	public void notifyEvent(OscarConnection connection) {
		if (getMessageType().getType() == MessageTypeEnum.PLAIN_MESSAGE)
			...
			log.error("UNRECOGNIZED IncomingMessage__4_7 TYPE: "
	...
	private void notifyIncomingMessage(OscarConnection connection) {
		...
		for (int i = 0; i < connection.getMessagingListeners().size(); i++) {
			MessagingListener l = (MessagingListener) connection
					.getMessagingListeners().get(i);
			log.debug("notify listener " + l.getClass().getName()
					+ " onIncomingMessage()");
	...
	private void parseType1(int position, byte[] data)
		...
		if ((getEncoding() & UCS2BE_ENCODING_MASK) > 0)
			message = StringTools.ucs2beByteArrayToString(data, position,
					msgLen);
		else
	...
	private void parseType2(int position, byte[] data)
		...
		if (ackType.getValue() != 0) {
			...
			if (ackType2.getValue() == 2) {
				...
				else if ((msgType.getValue() >= 0xe8)
						&& (msgType.getValue() <= 0xec))
	...
	private void parseType2Message(int position, byte[] data)
			throws ConvertStringException {
		/*
		 * Skipping 4 bytes till message (finally !)[i guess you could make it a
		 * lil bit more complicated mr AOL !]
		 */
		position += 4;
		...
		/* Retreiving message */
		message = StringTools.utf8ByteArrayToString(data, position,
				msgLen.getValue() - 1);
