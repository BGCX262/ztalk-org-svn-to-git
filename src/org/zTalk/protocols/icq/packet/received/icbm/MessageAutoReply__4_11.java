...
import org.zTalk.protocols.icq.setting.enumerations.MessageFlagsEnum;
import org.zTalk.protocols.icq.setting.enumerations.MessageTypeEnum;
...
public class MessageAutoReply__4_11 extends ReceivedPacket {
	...
	public MessageAutoReply__4_11(byte[] array) throws Exception {
		...
		if (messageChannel.getValue() == 2
				&& messageType.getType() == MessageTypeEnum.XSTATUS_MESSAGE) {
	...
	private void getChannel2Msg(byte[] data, int index)
			throws ConvertStringException {
		// 1st part
		RawData part1Lg = new RawData(data, index, RawData.WORD_LENGHT); // Little
																			// endian
																			// !
		index += RawData.WORD_LENGHT;
		RawData protocolVersion = new RawData(data, index, RawData.WORD_LENGHT); // Little
																					// endian
																					// !
		index += RawData.WORD_LENGHT;
		...
		index += RawData.WORD_LENGHT;
		RawData clientCapabilities = new RawData(data, index,
				RawData.DWORD_LENGHT); // Little endian !
		index += RawData.DWORD_LENGHT;
		RawData unknownByte = new RawData(data, index, RawData.BYTE_LENGHT);
		index += RawData.BYTE_LENGHT;
		RawData cntr1 = new RawData(data, index, RawData.WORD_LENGHT); // Little
																		// endian
																		// !
		index += RawData.WORD_LENGHT;

		// 2nd part
		RawData part2Lg = new RawData(data, index, RawData.WORD_LENGHT); // Little
																			// endian
																			// !
		index += RawData.WORD_LENGHT;
		RawData cntr2 = new RawData(data, index, RawData.WORD_LENGHT); // Little
																		// endian
																		// !
	...
	private String parseMessageType2(byte[] data, int index)
		...
		RawData status = new RawData(data, index, RawData.WORD_LENGHT); // Little
																		// endian
																		// !
		index += RawData.WORD_LENGHT;
		RawData priority = new RawData(data, index, RawData.WORD_LENGHT); // Little
																			// endian
																			// !
		index += RawData.WORD_LENGHT;
		RawData msgLg = new RawData(data, index, RawData.WORD_LENGHT); // Little
																		// endian
																		// !
		index += RawData.WORD_LENGHT;
		...
		message = StringTools.utf8ByteArrayToString(data, index,
				msgLg.getValue());
		...
		if (messageType.getType() == MessageTypeEnum.PLAIN_MESSAGE) {
			...
			index += RawData.DWORD_LENGHT;
			RawData backgroundColor = new RawData(data, index,
					RawData.DWORD_LENGHT);
			index += RawData.DWORD_LENGHT;
			// TODO fix this invocation exception
			/*
			 * RawData guidLg = new RawData(data, index, RawData.DWORD_LENGHT);
			 * index += RawData.DWORD_LENGHT; RawData guid = new RawData(data,
			 * index, guidLg.getValue());
			 */
		}
	...
	private void notifyXStatusRequest(OscarConnection connection) {
		...
		for (int i = 0; i < connection.getXStatusListeners().size(); i++) {
			XStatusListener l = (XStatusListener) connection
					.getXStatusListeners().get(i);
