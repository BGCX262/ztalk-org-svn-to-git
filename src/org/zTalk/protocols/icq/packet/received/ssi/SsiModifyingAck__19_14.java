 ...
 */
package org.zTalk.protocols.icq.packet.received.ssi;
...
public class SsiModifyingAck__19_14 extends ReceivedPacket {
	...
	public SsiModifyingAck__19_14(byte[] array) {
		...
		for (int i = 0; i < count; i++) {
			results[i] = new SsiResultModeEnum(new RawData(data, position,
	...
	public void notifyEvent(OscarConnection connection) {
		...
		for (int i = 0; i < connection.getContactListListeners().size(); i++) {
			ContactListListener l = (ContactListListener) connection
					.getContactListListeners().get(i);
