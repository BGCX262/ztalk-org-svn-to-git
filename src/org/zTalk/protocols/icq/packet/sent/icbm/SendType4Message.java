...
public class SendType4Message extends SendMessage {
	...
	public SendType4Message(String senderUin, String receiverUin, String message) {
		super(receiverUin, new MessageChannelEnum(
				MessageChannelEnum.MESSAGE_CHANNEL_4));

		Tlv tlv5 = new Tlv(0x05);

		// Sender ID
		RawData senderId = new RawData(Integer.parseInt(senderUin),
		...
		// Message type
		tlv5.appendRawDataToTlv(new RawData(MessageTypeEnum.PLAIN_MESSAGE,
				RawData.BYTE_LENGHT));

		// Message Flag
		tlv5.appendRawDataToTlv(new RawData(MessageFlagsEnum.NORMAL_MESSAGE,
				RawData.BYTE_LENGHT));

		// Message length
		RawData messageLength = new RawData(message.length(),
				RawData.WORD_LENGHT);
