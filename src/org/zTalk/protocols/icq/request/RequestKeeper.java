 ...
 */
package org.zTalk.protocols.icq.request;

import java.util.ArrayList;

import org.zTalk.protocols.icq.Flap;
import org.zTalk.protocols.icq.request.event.RequestListener;
...
public class RequestKeeper extends java.util.TreeMap {
	 ...
	 *            The request packet.
	 * @param listener
	 *            The listener to warn when the answer is received.
	 */
	public void addRequest(Flap packet, RequestListener listener) {
		if (packet.hasSnac()) {
			...
			if (containsRequest(packet.getSnac().getRequestId())) {
				Request request = (Request) this.getRequest(packet.getSnac()
						.getRequestId());
			...
			// TODO Maybe add some not blocking exception ex :
			// RequestOnNonSnacedFlapException
		}
	}

	public void addRequest(Request request) {
		/*
		 * If there is an existing request for the same requestId then we simply
		 * add the new requestListeners to the existing request object.
		 */
		if (containsRequest(request.getRequestId())) {
			Request existingRequest = (Request) this.getRequest(request
					.getRequestId());
			for (int i = 0; i < request.getNbListeners(); i++) {
				if (!existingRequest.containsListener(request
						.getRequestListener(i)))
	...
	/**
	 * This remove a listener from the list of listener that monitor a
	 * particular request packet.<br/>
	 * If there is no more listener monitoring the request, then the request is
	 * deleted.
	 * <p>
	 * This is equal to :
	 * <code>removeRequestListener(packet.getSnac().getRequestId(), listener);</code>
	 * </p>
	 * 
	 * @param packet
	 *            The packet that is monitored.
	 * @param listener
	 *            The listener that monitor the packet.
	 */
	 ...
	 * 
	 * @param requestId
	 *            The requestId that is monitored.
	 * @param listener
	 *            The listener that monitor the requestId.
	 */
	 ...
	 * 
	 * @param requestId
	 *            The request Id that is monitored.
	 * 
	 ...
	 * <p>
	 * This is equal to :
	 * <code>containsRequest(packet.getSnac().getRequestId());</code>
	 * </p>
	 * 
	 * @param packet
	 *            The packet containing the request Id that is monitored.
	 * 
	 ...
	 * 
	 * @param requestId
	 *            The request id we're searching for.
	 * 
	...
	/**
	 * Returns the request to which the requestId field of the snac packet is
	 * mapped.<br/>
	 * Returns null if there is no mapping for this request Id.
	 * 
	 * @param packet
